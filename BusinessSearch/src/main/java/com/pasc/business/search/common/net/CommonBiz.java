...
public class CommonBiz {
    ...
    public static Single<SearchHotBean> searchHot(String id) {
        BaseParam<HotParam> param = new BaseParam<> (new HotParam (id));
        RespV2Transformer<SearchHotBean> transformer = RespV2Transformer.newInstance ();
        return ApiGenerator.createApi (CommonApi.class).searchHot (param)
                .compose (transformer)
                .subscribeOn (Schedulers.io ())
                .observeOn (AndroidSchedulers.mainThread ());
    }
     ...
     * @return
     */
    public static Single<List<SearchThemeBean>> searchTheme(String entranceLocation) {
        BaseParam param = new BaseParam<> (new EntranceParam (entranceLocation));
        RespV2Transformer<List<SearchThemeBean>> transformer = RespV2Transformer.newInstance ();
        return ApiGenerator.createApi (CommonApi.class).searchTheme (param)
                .compose (transformer)
                .subscribeOn (Schedulers.io ())
                .observeOn (AndroidSchedulers.mainThread ());
    }
    ...
    public static Single<String> getSearchHint(final String idType) {
        return Single.create (new SingleOnSubscribe<String> () {
            ...
            public void subscribe(SingleEmitter<String> emitter) throws Exception {
                SearchInfoBean bean = SQLite.select ().from (SearchInfoBean.class).where (SearchInfoBean_Table.id.eq (idType.trim ())).querySingle ();
                String hintText = "";
                if (bean != null) {
    ...
    public static void getAllSearchHint() {
        ApiGenerator.createApi (CommonApi.class).searchHint (new BaseParam<VoidObject> (VoidObject.getInstance ()))
                .subscribeOn (Schedulers.io ())
