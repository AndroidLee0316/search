...
public class HomeBiz {
    ...
    public static void insertServiceLocalData(final String entranceLocation, final String appVersion, final String serviceVersion) {

        SearchBiz.queryCount (ItemType.personal_server, entranceLocation).flatMap (new Function<Long, SingleSource<ServiceResponse>> () {
            @Override
        ...
        }).observeOn (Schedulers.io ()).map (new Function<ServiceResponse, List<SearchSourceItem>> () {
            @Override
            public List<SearchSourceItem> apply(ServiceResponse serviceResponse) throws Exception {
                SharePreUtil.setString (Table.Prefer.pre_type_key + "_" + entranceLocation, serviceResponse.versionNo);
                    ...
                    if (serviceResponse.serviceList.size () > 0) {
                        ...
                        FlowManager.getDatabase (SearchSourceDB.class).executeTransaction (new ITransaction () {
                            ...
                            public void execute(DatabaseWrapper databaseWrapper) {
                                SQLite.delete (SearchSourceItem.class).where (SearchSourceItem_Table.type.eq (ItemType.personal_server))
    ...
    public static void insertUnionLocalData() {
        //先删除之前的数据,每次都更新
        SearchBiz.deleteAll (ItemType.personal_zhengqi_number, Table.Value.EntranceLocation.person_type).flatMap (new Function<Integer, SingleSource<List<UnionBean>>> () {
            @Override
            public SingleSource<List<UnionBean>> apply(Integer integer) throws Exception {
                return appStartUnionCache ();
            }
        }).observeOn (Schedulers.io ()).map (new Function<List<UnionBean>, List<SearchSourceItem>> () {
            @Override
    ...
    public static Single<ServiceResponse> appStartServiceCache(String entranceLocation, String appVersion, String serviceVersion) {
        return ApiGenerator.createApi (HomeApi.class).serviceSearchCache (new BaseParam<> (AppStartParam.instance (entranceLocation, appVersion, serviceVersion)))
    ...
    public static Single<List<UnionBean>> appStartUnionCache() {
        return ApiGenerator.createApi (HomeApi.class).unionSearchCache (new BaseParam<> (VoidObject.getInstance ()))
                .subscribeOn (Schedulers.io ())
